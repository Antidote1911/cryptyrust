...
fn do_it() -> Result<(Option<String>, Mode), Box<dyn Error>> {
    let matches = App::new("Cryptyrust")
        .version(get_version())
        .about("A simple file encryption utility. Passwords must be at least 8 characters. Copyright © 2021 Antidote1911. All rights reserved. https://github.com/Antidote1911/cryptyrust/")
        ...
        .arg(Arg::with_name("output")
            ...
            .help("Specifies a path or name for the output file. If the path to an existing directory is given, the input filename will be kept with the .crypty extension added if encrypting or removed (if decrypting). Otherwise the file will be placed and named according to this parameter.")
            .takes_value(true))
...
fn get_password(mode: &Mode) -> String {
    match mode {
        Mode::Encrypt => {
            let password = rpassword::prompt_password_stdout("Password (minimum 8 characters, longer is better): ")
